...
def supabase_health_check():
    ...
    with db() as con:
        ...
        tables = {}
        for table in (
            "members", "contributions", "loans", "loan_installments",
            "loan_votes", "member_messages", "member_reminders",
        ):
            found = con.execute(
...
def ensure_communication_schema():
    ...
    if not use_supabase():
        return
    with db() as con:
        con.execute("""CREATE TABLE IF NOT EXISTS public.loan_votes (
            id BIGSERIAL PRIMARY KEY, loan_id BIGINT NOT NULL REFERENCES public.loans(id) ON DELETE CASCADE,
            voter_member_id BIGINT NOT NULL REFERENCES public.members(id) ON DELETE CASCADE,
            decision TEXT NOT NULL DEFAULT 'En attente', comment TEXT,
            created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), UNIQUE (loan_id, voter_member_id)
        )""")
        con.execute("""CREATE TABLE IF NOT EXISTS public.member_messages (
